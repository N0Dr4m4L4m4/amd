...
DROP FUNCTION IF EXISTS show_video_role(TEXT, INT, TEXT, TEXT);
DROP FUNCTION IF EXISTS show_not_to_film_related_persons(TEXT, INT);
DROP FUNCTION IF EXISTS edit_video_role(TEXT, INT, TEXT, TEXT, TEXT[]);
DROP FUNCTION IF EXISTS del_video_role(TEXT, INT, TEXT, TEXT);
...
CREATE OR REPLACE FUNCTION change_username(IN old_benutzer_name TEXT, IN new_benutzer_name TEXT) RETURNS TEXT AS $$
  BEGIN
    ...
    ELSIF (new_benutzer_name = ANY(SELECT name FROM benutzer)) THEN
      return 'Dieser Benutzer existiert bereits. Nicht änderbar!';
    ELSE
      UPDATE benutzer
...
-- Anzeige aller Personen, die noch nicht in bestimmten Film mitwirken, zum Linken einer neuen Person zu einem Film --
CREATE OR REPLACE FUNCTION show_not_to_film_related_persons(IN tit TEXT, IN rel INT) RETURNS TABLE(Nachname TEXT, Vorname TEXT) AS $$
  BEGIN
    RETURN QUERY
      SELECT a.Nachname, a.Vorname FROM show_all_persons() a
    EXCEPT 
      SELECT f.Nachname, f.Vorname FROM show_to_film_related_persons(tit , rel) f
      ORDER BY Nachname, Vorname; -- gibt aus was in 1. menge und nicht in 2. menge enthalten ist
  END;
$$ LANGUAGE plpgsql;

-- Zu einem Film eine mitwirkende Person einfügen --
CREATE OR REPLACE FUNCTION edit_video_role(IN tit TEXT, IN rel INT, IN sur TEXT, IN forn TEXT, IN rol TEXT[]) RETURNS TEXT AS $$
  BEGIN
    IF EXISTS(SELECT * FROM film_related_person WHERE surname=sur AND forname=forn) THEN
      IF EXISTS(SELECT * FROM acts WHERE surname=sur AND forname=forn AND title=tit AND release_year=rel) THEN
        UPDATE acts
          SET role=rol
            WHERE surname=sur AND forname=forn AND title=tit AND release_year=rel;
        RETURN 'Rollen erfolgreich geupdated';
      ELSE
...
CREATE OR REPLACE FUNCTION show_films_seasons(IN ser TEXT) RETURNS table(Liste TEXT[]) AS $$
  BEGIN
    IF ((SELECT season_nr FROM video where series_name = ser LIMIT 1) IS NULL) THEN
      RETURN QUERY
      SELECT ARRAY[title, CAST(release_year as TEXT)] FROM video 
        WHERE series_name = ser
	ORDER BY episode_nr, series_name;
    ELSE
      RETURN QUERY
      SELECT DISTINCT(ARRAY[CAST(season_nr as TEXT)]) as staffeln from video 
        WHERE series_name = ser
	      ORDER BY staffeln;
    END IF;
  END;
...
CREATE OR REPLACE FUNCTION show_episodes(IN ser TEXT, IN sea INT) RETURNS TABLE(Episodennr INT, Episodenname TEXT, Erscheinungsjahr INT) AS $$
  BEGIN
    RETURN QUERY
    SELECT episode_nr, title, release_year FROM video
...
CREATE OR REPLACE FUNCTION show_to_film_related_persons(IN tit TEXT, IN rel INT) RETURNS TABLE (Nachname TEXT, Vorname TEXT, Rollen TEXT[]) AS $$
  BEGIN 
    RETURN QUERY
    SELECT a.surname, a.forname, a.role FROM video v 
      JOIN acts a ON (a.title=v.title AND a.release_year=v.release_year)
      WHERE v.title = tit AND v.release_year = rel
      ORDER BY a.surname, a.forname;
  END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION del_video(IN tit TEXT, IN rel INT) RETURNS TEXT AS $$
  BEGIN
    ...
    RETURN CONCAT(E'Folgender Film wurde gelöscht:\n  Titel: ', tit, E'\n  Erscheinungsjahr: ' ,rel); --return deleted element
  END;
...
CREATE OR REPLACE FUNCTION suggestion_all(IN nam TEXT) RETURNS TABLE(Resultat NUMERIC, Titel TEXT, Erscheinungsjahr INT)
AS $$
  BEGIN
    RETURN QUERY
    SELECT ROUND(AVG(s.Durchschnitt), 2) AS Resultat, v.title, v.release_year
...
CREATE OR REPLACE FUNCTION suggestion(IN nam TEXT) RETURNS TABLE(Resultat NUMERIC, Titel TEXT, Erscheinungsjahr INT)
AS $$
  BEGIN
    RETURN QUERY
    SELECT s.*
...
--select * from change_username('Axel', 'Axelino'); --Name geändert
--select * from change_username('raineirn', 'Axelino'); --Benutzer raineirn existiert nicht
--select * from change_username('Axel', 'Axel'); --Name ist identisch
--select * from change_username('Axel', 'Robin'); --Robin existiert bereits als user. nicht änderbar!
...
-- Anzeige einer Person und deren Rollen in einem bestimmten Film --
--select * from show_video_role('Geheimnisse', 2017, 'Hofmann', 'Luis');
--select * from show_video_role('Der Pate', 1972, 'Pacino', 'Al');

-- Anzeige Personen, die noch nicht zu bestimmten Film gelinked sind--
--select * from show_not_to_film_related_persons('Harry Potter und der Stein der Weisen', 2001)

-- Aufruf Funktion zum Hinzufügen eines Films samt im Film mitwikrender Person --
-- then sollte Eintrag eingefügt werden
-- falls bereits existent, dann sollte NULL zurückgegeben werden -> "Title existiert bereits"
--select * from insert_video('Harry Potter und der Test',2001,'{Fantasy,Abenteuer}',12,140, 1, NULL, 'Harry Potter');
--select * from edit_video_role('Harry Potter und der Test',2001,'Watson','Emma', '{Schauspieler, Statist}');
--select * from insert_video('Zweite Dark Folge',2017,'{Science-Fiction,Mystery,Thriller}',12,52, 2, 1, 'Dark');
--select * from insert_video('Geheimnisse',2017,'{Science-Fiction,Mystery,Thriller}',12,52, 2, 1, 'Dark');

-- Lösche Rollen einer Person aus einem Film --
--select * from del_video_role('Geheimnisse', 2017, 'Hofmann', 'Luis');
...
-- falls Person schon vorhanden, then NULL als Rückgabe -> 'diese Person ist bereits vorhanden'
--select * from insert_person('Tester', 'Hans', '1999-12-12', 'd')
--select * from insert_person('Watson', 'Emma', '1999-12-12', 'd')
